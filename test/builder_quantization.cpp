...
TEST(builder, scaled_QC_s8s8)
{
    Shape shape_a{1, 1, 3, 4}; // input shape
    Shape shape_b{1, 1, 3, 3}; // filter shape
    Shape shape_r{1, 1, 3, 4}; // output shape
    vector<int8_t> a_data = {1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4};//{-1, -2, 3, 2, 4, 1, 0, 1, 0};
    vector<int8_t> b_data = {1, 2, 3, 4, 5, 0, 0, 1, 2};//{0, -1, 0, -2, -3, 5, 0, 2, 1};
    auto A = make_shared<op::Parameter>(element::i8, shape_a);
    auto B = make_shared<op::Parameter>(element::i8, shape_b);
    ...
    auto handle = backend->compile(f);
    handle->call_with_validate({result}, {a, b});
    EXPECT_EQ((vector<int8_t>{22, 34, 30, 32, 38, 72, 90, 43, 33, 52, 43, 39}/*{1, 28, -3, 16, -7, -14, 3, -7, -3}*/),
              read_vector<int8_t>(result));
}
