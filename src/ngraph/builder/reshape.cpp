...
Output<Node> builder::reshape(const Output<Node>& node, const Shape& shape)
{
    return make_shared<op::Reshape>(node, get_default_order(node.get_shape().size()), shape);
}

Output<Node> builder::reorder_axes(const Output<Node>& node, vector<size_t> axes_order)
{
...
Output<Node> builder::transpose(const Output<Node>& node)
{
...
Output<Node> builder::flatten(const Output<Node>& node, int axis)
{
